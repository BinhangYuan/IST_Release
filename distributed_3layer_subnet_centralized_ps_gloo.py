    ...
    def partition_to_list(self):
        ...
        for i in range(self.partition_num):
            current_indexes = torch.tensor(
                self.temp_hidden_layer_index1[i * self.partition_dim:(i + 1) * self.partition_dim])
        ...
        for i in range(self.partition_num):
            current_indexes = torch.tensor(
                self.temp_hidden_layer_index2[i * self.partition_dim:(i + 1) * self.partition_dim])
...
def worker_process(args):
    ...
    epochs = args.epochs * args.partition_group
...
def parameter_server_process(args):
    ...
    raw_model = DNNGoogleSpeechBatchNorm3Layer(partition_num=args.partition_group,
